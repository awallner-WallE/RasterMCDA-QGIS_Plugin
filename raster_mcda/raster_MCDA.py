...
#------------------------Updates OWA--------------------------------------------------
    ...
    def getOrderedWeights(self):
        ...
        if( (oWeights != np.sort(oWeights)).all() or (oWeights != np.sort(oWeights)[::-1]).all() ):
        ...
        try:
            ...
            for line in wpcFile:
                ...
                if factorname == "Consistency":
                    self.weightsLoadet = True
                    self.dlg.showMessageBox(QMessageBox.Information, "Information" , "Weights loadet!", "Consistency ratio of weighting was: "+str(weight), "")
